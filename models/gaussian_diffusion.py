...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, model, x_start, steps, sampling_noise=False, cond_vec=None):
        """
        Updated to support conditional diffusion with similarity vectors
        """
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                # Pass conditional vector if provided
                if cond_vec is not None:
                    x_t = model(x_t, t, cond_vec)
                else:
                    x_t = model(x_t, t)
            return x_t
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            # Pass conditional vector to p_mean_variance
            if cond_vec is not None:
                out = self.p_mean_variance(model, x_t, t, cond_vec)
            else:
    ...
    def training_losses(self, model, x_start, reweight=False, cond_vec=None):
        """
        Updated to support conditional diffusion with similarity vectors
        """
        ...
        # Pass conditional vector if provided
        if cond_vec is not None:
            model_output = model(x_t, ts, cond_vec)
        else:
        ...
        if reweight == True:
            if self.mean_type == ModelMeanType.START_X:
                ...
                # Compute likelihood
                if cond_vec is not None:
                    pred_xstart = self._predict_xstart_from_eps(x_t, ts, model_output)
                else:
                    pred_xstart = self._predict_xstart_from_eps(x_t, ts, model_output)
                likelihood = mean_flat((x_start - pred_xstart)**2 / 2.0)
    ...
    def p_mean_variance(self, model, x, t, cond_vec=None):
        """
        Updated to support conditional diffusion with similarity vectors
        """
        B, C = x.shape[:2]
        ...
        # Pass conditional vector if provided
        if cond_vec is not None:
            model_output = model(x, t, cond_vec)
        else:
