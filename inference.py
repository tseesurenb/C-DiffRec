...
# params for similarity-based conditioning
parser.add_argument('--use_similarity', action='store_true', help='use similarity conditioning')
parser.add_argument('--similarity_matrix_path', type=str, default=None, help='pre-computed similarity matrix path')
parser.add_argument('--similarity_method', type=str, default='cosine', help='method to compute similarity: cosine, jaccard')
parser.add_argument('--similarity_normalization', type=str, default='none', help='similarity normalization method: none, min_max, l2')
...
# If similarity matrix path is provided, use it, otherwise compute from data
if args.similarity_matrix_path:
    sim_matrix_path = args.similarity_matrix_path
else:
    sim_matrix_path = args.data_path + 'similarity_matrix.npy'

# Load data with similarity matrix if it exists and if similarity conditioning is enabled
if args.use_similarity and os.path.exists(sim_matrix_path):
    train_data, valid_y_data, test_y_data, n_user, n_item, similarity_matrix = data_utils.data_load(
        train_path, valid_path, test_path, sim_matrix_path)
else:
...
if args.use_similarity and similarity_matrix is None:
    print(f"Computing {args.similarity_method} similarity matrix...")
...
if args.use_similarity and similarity_matrix is not None:
    train_dataset = data_utils.DataDiffusion(
        torch.FloatTensor(train_data.A), 
        similarity_matrix=similarity_matrix,
        normalization_method=args.similarity_normalization
    )
else:
...
def evaluate(data_loader, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            # Handle both regular and similarity-conditioned data loaders
            if args.use_similarity and similarity_matrix is not None:
                batch_data, similarity_vec = batch
                batch_data = batch_data.to(device)
                similarity_vec = similarity_vec.to(device)
                prediction = diffusion.p_sample(model, batch_data, args.sampling_steps, args.sampling_noise, cond_vec=similarity_vec)
            else:
                if isinstance(batch, tuple):
                    batch_data = batch[0].to(device)
                else:
                    batch_data = batch.to(device)
                prediction = diffusion.p_sample(model, batch_data, args.sampling_steps, args.sampling_noise)
